...
在 **Tomcat** 中，Session 是服务器端为每个客户端会话分配的一块内存空间，用于存储会话期间的状态数据，其底层结构是 **ConcurrentHashMap**。由于 **HTTP 协议是无状态的**，Session 的主要作用是弥补这一缺陷，让服务器能够识别并保存同一用户在多次请求间的状态信息。

**会话标识与传递** Tomcat 使用 **JSESSIONID** 作为会话标识。
...
public class UserService extends ServiceImpl<UserMapper, User> {
     ...
     * 使用Lambda查询（更安全）
     */
    public User getByPhoneLambda(String phone) {
        return lambdaQuery()
     ...
     * 多条件查询
     */
    public List<User> getUsersByConditions(String name, Integer minAge, Integer maxAge) {
        return lambdaQuery()
     ...
     * 分页查询
     */
    public Page<User> getUserPage(Integer pageNum, Integer pageSize, String department) {
        Page<User> page = new Page<>(pageNum, pageSize);
        return lambdaQuery()
            .eq(StringUtils.isNotBlank(department), User::getDepartment, department)
            .orderByDesc(User::getCreateTime)
            .page(page);
    }
     ...
     * Lambda方式更新
     */
    public boolean updateUserInfo(Long id, String name, Integer age) {
        return lambdaUpdate()
     ...
     * 条件删除
     */
    public boolean deleteInactiveUsers() {
        return query()
            .eq("status", 0)
            .lt("update_time", LocalDateTime.now().minusMonths(6))
            .remove();
    }
     ...
     * Lambda方式删除
     */
    public boolean deleteByConditions(Integer maxAge, String department) {
        return lambdaQuery()
            .lt(User::getAge, maxAge)
     ...
     * 子查询
     */
    public List<User> getUsersInDepartment(List<String> departments) {
        return query()
            .inSql("department", 
                "SELECT name FROM department WHERE type = '技术'")
            .eq("status", 1)
            .list();
    }
    ...
    public List<User> getComplexQueryUsers() {
        return lambdaQuery()
            ...
            .and(q -> q.like(User::getName, "张%").or().like(User::getName, "李%"))
     ...
     */
    @Transactional
    public void batchImportUsers(List<UserImportDTO> dtos) {
...
public class UserService extends ServiceImpl<UserMapper, User> {
    ...
    // 3.1 更多 query() 方法
    public void moreQueryMethods() {
        // 获取一条记录，多条则报错
        User user = query().eq("phone", "13800138000").one();
        
        // 获取第一条
        User firstUser = query().eq("status", 1).orderByAsc("id").one();
        
        // 查询指定字段
        List<Map<String, Object>> maps = query()
        ...
        List<String> names = query()
            .select("name")
            .eq("status", 1)
            .listObjs(obj -> (String) obj);
        
        // 去重查询
        List<String> departments = query()
            .select("DISTINCT department")
            .listObjs(obj -> (String) obj);
        
        // 使用函数
        List<Map<String, Object>> result = query()
            .select("COUNT(*) as count", "MAX(age) as max_age", "MIN(age) as min_age")
            .listMaps();
    }
    
    // 3.2 更多 update() 方法
    public void moreUpdateMethods() {
        // 递增递减操作
        boolean updated = update()
            .setSql("age = age + 1")
            .setSql("points = points + 10")
            .eq("id", 1L)
            .update();
        
        // 条件递增
        boolean updated2 = lambdaUpdate()
...
**由于负载均衡,一个用户可能访问不同的服务器,但session本身只在一个tomcat的内存当中,故此时并不共享,导致登录出现问题**

**使用redis解决,多个tomcat只连接一个redis**
...
之前的方案,**只有访问拦截的路径才去刷新token的存活期限**,在我们访问一些不需要拦截的路径时就不会刷新了

在这个方案中，他确实可以使用对应路径的拦截，同时**刷新登录token令牌**的存活时间，**但是现在这个拦截器他只是拦截需要被拦截的路径，假设当前用户访问了一些不需要拦截的路径，那么这个拦截器就不会生效**，所以此时令牌刷新的动作实际上就不会执行，所以这个方案他是存在问题的
...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        //因为前置的拦截器已经将存在的用户保存在ThreadLocal中,这里直接判断TL里面有没有东西就行
        //1个请求必定只有一个用户
        //前面拦截的有token,说明就不是登录请求了,就直接验证token是否有效,有效就将其放入TL中
		//若前面拦截的请求有token但token在redis中不存在，则说明token已经过期，需要拦截
...
**因为缓存执行的很快,容易被其他人抢掉进程,而操作数据库慢,紧着就操作缓存了,不容易被抢**,**而且方案1,会造成长时间的错误,而方案二即使在更新完数据库后,突然查询,命中缓存,是旧的,也会马上删除缓存,别人只要再查询就会更新,而方案1则需要等待别人再一次的更新操作,这个时间就久了**
...
public void updateUserWithDelayDelete(User user) {
    ...
    // 3. 延迟一段时间后再次删除缓存
    delayDeleteCache("user:" + user.getId(), 500); // 延迟500ms
}
```

对于读写严格的场景,**银行,直接上锁去一起执行**
...
1. **该数据是热点数据**，应该已经被预加载到缓存
2. **缓存未命中意味着数据不存在或不是热点**
3. **为了性能，不查询数据库**

思路分析：当用户开始查询redis时，判断是否命中，如果没有命中则直接返回空数据,**因为只用于解决热点问题,不命中说明就不是热点的,使用其他办法即可,比如互斥锁**，不查询数据库，而一旦命中后，将value取出，判断value中的过期时间是否满足，如果没有过期，则直接返回redis中的数据，如果过期，则在开启独立线程后直接返回之前的数据，独立线程去重构数据，重构完成后释放互斥锁。
...
之前的方式要修改前后都保持一致，但是这样我们分析过，成功的**概率太低**，所以我们的乐观锁需要变一下，**改成stock大于0** 即可

在方案二中，即使多个线程同时通过了库存大于 0 的检查，但由于**数据库更新操作的原子性**，只有第一个线程能够成功扣减库存。其他线程在尝试扣减库存时，会发现库存不足并返回失败，从而避免了超卖问题
...
**判断和删除并不是一定会一起执行!即非原子性,一旦有阻塞就会导致误删问题**
...
重入就是指,同一个线程能否多次获得同一把锁,redis做不到!因为是简单的String类型,

```
# 第一次获取锁成功
SETNX lock:order 1  # 成功返回1

# 同一个线程内再次尝试获取同一个锁
SETNX lock:order 1  # 失败返回0，因为键已存在
```

而redission可以做到,因为它使用了Hash结构

```
# key: 锁的名称
# field: 客户端/线程的唯一标识（如UUID:threadId）
# value: 重入次数
HGETALL lock:order
...
public boolean tryLock() {
    // 判断锁是否已存在
    Object currentLockHolder = redis.hget(lockKey, "lockHolder");
    
    if (currentLockHolder == null) {
        // 锁不存在，直接获取
        redis.hset(lockKey, clientThreadId, 1);
        redis.pexpire(lockKey, ttl);
        return true;
    } else if (currentLockHolder.equals(clientThreadId)) {
        // 同一个线程，重入次数+1
        redis.hincrby(lockKey, clientThreadId, 1);
        return true;
    } else {
        // 其他线程持有锁，获取失败
        return false;
...
public void unlock() {
    // 获取当前重入次数
    Integer count = redis.hget(lockKey, clientThreadId);
